...
impl PackageImportReport {
    ...
    pub fn found_category_late_with_details(&mut self, full_category_name: &String, category_uuid: Uuid, requester_uuid: &Uuid, source_file_name: &String) {
        ...
        if !self._missing_categories_tracker.contains(full_category_name) {
            self.number_of.missing_categories += 1;
            self._missing_categories_tracker.insert(full_category_name.clone());
        }
    }
        ...
        if let Some(category_uuid) = self.all_categories.get(full_category_name) {
            ...
            while let Some(parent_full_category_name) = prefix_until_nth_char(&full_category_name, '.', n) {
                ...
                if let Some(parent_uuid) = self.all_categories.get(&parent_full_category_name) {
                    ...
                    self.report.found_category_late(&full_category_name, *parent_uuid);
