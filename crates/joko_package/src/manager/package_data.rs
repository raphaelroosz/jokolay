...
use joko_link::{MumbleLink, MumbleLinkSharedState};
...
use super::pack::loaded::{jokolay_to_marker_dir, jokolay_to_marker_path};
...
impl PackageDataManager {
    ...
    pub fn new(
        ...
        let marker_packs_dir = jokolay_to_marker_dir(&root_dir)?;
        let marker_packs_path = jokolay_to_marker_path(root_path);
        //TODO: load configuration from disk (ui.toml)
        let editable_path = jokolay_to_editable_path(root_path.clone())
            .to_str()
            .unwrap()
            .to_string();
            ...
            MessageToPackageBack::ImportPack(file_path) => {
                ...
                match result {
                    Ok((file_name, pack)) => {
                        let _ = self
                            .channel_sender
                            .send(MessageToPackageUI::ImportedPack(file_name, pack).into());
                    }
                    Err(e) => {
                        let _ = self
                            .channel_sender
                            .send(MessageToPackageUI::ImportFailure(e).into());
                    }
                }
                let _ = self
                    .channel_sender
                    .send(MessageToPackageUI::NbTasksRunning(0).into());
            }
            ...
            MessageToPackageBack::SavePack(name, pack) => {
                ...
                match marker_packs_dir.open_dir(name) {
                    Ok(dir) => {
                        ...
                        let _ = self
                            .channel_sender
                            .send(MessageToPackageUI::LoadedPack(texture_pack, report).into());
                    }
                    Err(e) => {
                        error!(
                            ?e,
    ...
    pub fn tick(
        ...
        if let Some(link) = link {
            ...
            for pack in self.packs.values_mut() {
                ...
                let _ = self
                    .channel_sender
                    .send(MessageToPackageUI::NbTasksRunning(tasks.count()).into());
            ...
            if map_changed {
                ...
                let _ = self
                    .channel_sender
                    .send(MessageToPackageUI::ActiveElements(self.loaded_elements.clone()).into());
            }
            if map_changed || have_used_files_list_changed || self.state.choice_of_category_changed
            {
                ...
                let _ = self
                    .channel_sender
                    .send(MessageToPackageUI::TextureSwapChain.into());
            }
        }
    ...
    pub fn load_all(&mut self) {
        ...
        let _ = self
            .channel_sender
            .send(MessageToPackageUI::FirstLoadDone.into());
    }
}
